...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    *************************************************************************/
    ...
    public int numberOfRed(int count)
    {
    ...
    private int numberOfRed(Node node,  int count)
    {
    	...
    	else if(isRed(node))
    	{
    		...
    		return 1 + numberOfRed(node.right, count) + numberOfRed(node.left, count);
    	}
    	else
    	{
    		StdOut.println("BLACK");
    		return numberOfRed(node.right, count) + numberOfRed(node.left, count);
    	}
    }
    public double precentageOfRed(int count)
    {
    	StdOut.println("root.N: " + root.N);
    	StdOut.println("count: " + count);
    	double percentage = (double)count/(root.N+1);
    	StdOut.println(percentage + "%" + "    " + count/root.N);
    	return percentage;
    }
...
/*public static void main(String[] args) { 
    ...
    for(int j = 0; j < N; j++)
    {
    	for (int i = 0; i < N; i++)
    	{
    		int key2 = StdRandom.uniform(N);
    		String key = Integer.toString(key2);
    		int value = StdRandom.uniform(N);
        	st.put(key, value);
    	}
    	int count = 0;
    	count = numberOfRed(root, count);
    	StdOut.println("percentage of red in given tree: " + count/root.N + "%" + " in round: " + j);
...
public int numberOfRed(Node node,  int count)
{
	...
	{
		return ++count + numberOfRed(node.right, count) + numberOfRed(node.left, count);
	}
	else
	{
		return numberOfRed(node.right, count) + numberOfRed(node.left, count);
